...
def plot_3d_motion(motion, length, save_path, params, title="", interval=50, palette=None):
    ...
    def update(index):
        # Safely clear all 3D lines and objects
    ...
    if length <= 0 or motion.shape[2] < length:
        print(f"[Warning] Invalid length ({length}) or insufficient frames in motion ({motion.shape[2]}). Skipping animation for {save_path}")
        return
    ani = FuncAnimation(fig, update, frames=length, interval=interval, repeat=False, init_func=init)

    plt.tight_layout()
    if length > 0 and motion.shape[2] >= length:
        ani.save(save_path, writer='pillow', fps=1000/interval)
    else:
        print(f"[Warning] Skipping animation save for {save_path} due to insufficient motion frames.")
    plt.close()
