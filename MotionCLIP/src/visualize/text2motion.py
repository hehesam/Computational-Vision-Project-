...
def main():
    ...
    # Fix for invalid 'cuda:None' issue
    if parameters.get("cpu", False):
        parameters["device"] = "cpu"
    elif parameters.get("cuda", False):
        parameters["device"] = "cuda:0"
    else:
        print("No device specified, using CPU as fallback.\n\n")
        parameters["device"] = "cpu"  # fallback
